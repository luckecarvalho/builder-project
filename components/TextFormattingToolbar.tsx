...
const TextFormattingToolbar: React.FC<TextFormattingToolbarProps> = ({
  ...
  const fontSizesPreset = [12, 14, 16, 18, 20, 24, 28, 32, 36, 48];
  const fontSizeList = React.useMemo(() => {
    if (currentFormats.fontSizeMixed) {
      return fontSizesPreset;
    }
    const cur = currentFormats.fontSize;
    if (cur != null && !fontSizesPreset.includes(cur)) {
      return [...fontSizesPreset, cur].sort((a, b) => a - b);
    }
    return fontSizesPreset;
  }, [currentFormats.fontSize, currentFormats.fontSizeMixed]);

  const displayedFontSize =
    currentFormats.fontSize != null && currentFormats.fontSize > 0
      ? currentFormats.fontSize
      : 16;

  const fontSizeToolbarLabel = currentFormats.fontSizeMixed
    ? '-'
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <div className="relative flex items-center space-x-1">
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-xs border border-gray-300 rounded hover:bg-gray-50 min-w-[3.5rem]"
            title="Tamanho da fonte"
          >
            <span className="text-gray-700 min-w-[2.25rem] text-center">{fontSizeToolbarLabel}</span>
              ...
              {fontSizeList.map(size => (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 text-sm hover:bg-gray-100 ${
                    !currentFormats.fontSizeMixed && displayedFontSize === size
        ...
        <div className="relative">
          ...
          >
            ...
            {currentFormats.colorMixed ? (
              <span className="inline-flex items-center justify-center w-4 h-4 rounded border border-gray-300 text-[10px] text-gray-500 font-medium">
                -
              </span>
            ) : (
              <span
                ...
                {colors.map(color => (
                  <button
                    ...
                    className={`w-6 h-6 rounded border ${
                      !currentFormats.colorMixed && currentFormats.color === color
                        ? 'border-blue-500'
                        : 'border-gray-300'
                    }`}
        ...
        <div className="relative">
          ...
          >
            ...
            {currentFormats.backgroundColorMixed ? (
              <span className="inline-flex items-center justify-center w-4 h-4 rounded border border-gray-300 text-[10px] text-gray-500 font-medium">
                -
              </span>
            ) : (
              ...
              <div className="grid grid-cols-8 gap-1 mb-3">
                {colors.map(color => (
                  <button
                    ...
                    className={`w-6 h-6 rounded border ${
                      !currentFormats.backgroundColorMixed &&
                      currentFormats.backgroundColor === color
                        ? 'border-blue-500'
                        : 'border-gray-300'
                    }`}
                    style={{ backgroundColor: color }}
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  value={
                    currentFormats.backgroundColorMixed
                      ? '#ffffff'
                      : currentFormats.backgroundColor || '#ffffff'
                  }

...
const PropertyPanel: React.FC<PropertyPanelProps> = ({ block, isPreview = false, onUpdate, onClose }) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center">
        <input
          ...
          className="mr-2"
        />
        <span className="text-sm text-gray-700">Carregamento Lazy</span>
      </div>
    </div>
  );
  ...
  const renderCarouselProperties = () => {
    ...
    const toggleAllCardsFeature = (feature: 'image' | 'button') => {
      const newItems = [...items];
      ...
      if (feature === 'image') {
        ...
        const shouldEnable = !currentCard.button;
        newItems.forEach((item, index) => {
          if (shouldEnable) {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-3 pt-4 border-t">
          ...
          {/* Configuração de Botão Global */}
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center gap-2">
              <MousePointer className="w-5 h-5 text-green-600" />
              <div>
                <p className="text-sm font-medium text-gray-700">Habilitar Botão</p>
                <p className="text-xs text-gray-500">Aplica a todos os cards • Clique no botão para configurar</p>
              </div>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={items.some((item: any) => item.button)}
                onChange={() => toggleAllCardsFeature('button')}
                className="sr-only peer"
              />
        ...
        <div className="space-y-3 pt-4 border-t">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">Avançar automaticamente</p>
            </div>
            <input
              type="checkbox"
              checked={content.autoplay || false}
